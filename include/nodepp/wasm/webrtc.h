...
public:
    ...
    webrtc_t() : obj( new NODE() ) { obj->krn = process::NODEPP_EVLOOP(); }
   ...
   ~webrtc_t() { if( obj.count()>1 ){ return; } free(); }
    ...
    bool    is_closed() const noexcept { return  is_state ( STATE::FS_STATE_DISABLE ) || obj->state==0x00 || obj->dc.isUndefined(); }
