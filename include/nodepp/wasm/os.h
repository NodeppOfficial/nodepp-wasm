...
namespace nodepp { namespace os {
    ...
    inline uchar_64 rand() { 
        auto out = EM_EVAL( "return `${Math.random()}`.slice(2)" ).as<EM_STRING>();
        return string::to_u64( string_t( out ) ); 
    }
    ...
    inline void reset(){ EM_EVAL( "window.location.reload()" ); }
    ...
    inline string_t cwd(){ return string_t( EM_EVAL( "Module.FS.cwd()" ).as<EM_STRING>() ); }
