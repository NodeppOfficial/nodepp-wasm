...
public:
   ...
   ~ws_t() noexcept { if( obj.count() > 1 ) { return; } free(); }
    ...
    ws_t() noexcept : obj( new NODE() ){ obj->krn = process::NODEPP_EVLOOP(); }
    ...
    bool    is_closed() const noexcept { return  is_state ( STATE::FS_STATE_DISABLE ) || obj->state==0x00 || obj->fd.isUndefined(); }
