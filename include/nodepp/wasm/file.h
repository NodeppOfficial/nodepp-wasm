...
protected:
    ...
    string_t get_fd_flag( const string_t& flag ){
        if  ( flag == "r"  ){ return "rb" ; }
        elif( flag == "w"  ){ return "wb" ; }
        elif( flag == "a"  ){ return "ab" ; }
        elif( flag == "r+" ){ return "rb+"; }
        elif( flag == "w+" ){ return "ab+"; }
        elif( flag == "a+" ){ return "ab+"; }
        else /*----------*/ { return "ab+"; }
    }
...
public:
    ...
    file_t( const string_t& path, const string_t& mode, const ulong& _size=NODEPP_CHUNK_SIZE ) : obj( new NODE() ) {
            obj->fd = fopen( path.c_str(), get_fd_flag(mode).get() ); 
        if( obj->fd == nullptr ){
    ...
    file_t( FILE* fd, const ulong& _size=NODEPP_CHUNK_SIZE ) : obj( new NODE() ) {
        if( fd == nullptr ){ 
            NODEPP_THROW_ERROR( except_t( "invalid FILE*" ) );
   ...
   ~file_t() noexcept { if( obj.count()>1 && !is_closed() ){ return; } free(); }
    ...
    virtual int __read( char* bf, const ulong& sx ) const noexcept {
        ...
        auto c = ::fread( bf, sizeof(char), sx, obj->fd );
        return c<=0 ? -1 : c;
    }

    virtual int __write( char* bf, const ulong& sx ) const noexcept {
        if( is_closed() ){ return -1; } if( sx==0 ){ return 0; }
        auto c = ::fwrite( bf, sizeof(char), sx, obj->fd );
        return c<=0 ? -1 : c;
    }
