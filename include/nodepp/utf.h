...
inline ptr_t<uchar_32> utf8_to_utf32( const ptr_t<uchar_8>& utf8 ) {
    queue_t<uchar_32> utf32 ; ulong i=0; while ( i < utf8.size() ) { uchar_8 byte = utf8[i];

        if (byte < 0x80) {
            utf32.push( byte ); i += 1;
        } elif ((byte & 0xE0) == 0xC0) {
            if (i + 1 >= utf8.size()){ return nullptr; }
            utf32.push( type::cast<uchar_32>(byte & 0x1F) << 6 | (utf8[i + 1] & 0x3F) );
            i += 2;
        } elif ((byte & 0xF0) == 0xE0) {
            if (i + 2 >= utf8.size()){ return nullptr; }
            utf32.push( type::cast<uchar_32>(byte & 0x0F) << 12 | (utf8[i + 1] & 0x3F) << 6 | (utf8[i + 2] & 0x3F) );
            i += 3;
        } elif ((byte & 0xF8) == 0xF0) {
            if (i + 3 >= utf8.size()){ return nullptr; }
            utf32.push( type::cast<uchar_32>(byte & 0x07) << 18 | (utf8[i + 1] & 0x3F) << 12 | (utf8[i + 2] & 0x3F) << 6 | (utf8[i + 3] & 0x3F) );
            i += 4;
        } else { return nullptr; }
...
inline ptr_t<uchar_16> utf8_to_utf16( const ptr_t<uchar_8>& utf8 ) {
    queue_t<uchar_16> utf16; ulong i = 0; while ( i<utf8.size() ) {
        ...
        if (byte < 0x80) {
            ...
            if (i+1 >= utf8.size()){ return nullptr; }
            codepoint = type::cast<uchar_32>(byte & 0x1F) << 6 | (utf8[i + 1] & 0x3F); 
            i += 2;
        } elif ((byte & 0xF0) == 0xE0) {
            if (i+2 >= utf8.size()){ return nullptr; }
            codepoint = type::cast<uchar_32>(byte & 0x0F) << 12 | (utf8[i + 1] & 0x3F) << 6 | (utf8[i + 2] & 0x3F);
            i += 3;
        } elif ((byte & 0xF8) == 0xF0) {
            if (i+3 >= utf8.size()){ return nullptr; }
            codepoint = type::cast<uchar_32>(byte & 0x07) << 18 | (utf8[i + 1] & 0x3F) << 12 | (utf8[i + 2] & 0x3F) << 6 | (utf8[i + 3] & 0x3F);
            i += 4;
        } else { return nullptr; }
...
inline ptr_t<uchar_32> utf16_to_utf32( const ptr_t<uchar_16>& utf16 ) {
    queue_t<uchar_32> utf32; ulong i = 0; while ( i<utf16.size() ) {
        ...
        if ( i+1>=utf16.size() ){ return nullptr; }
            uchar_16 low = utf16[i + 1];
        if ( low<0xDC00 || low>0xDFFF){ return nullptr; }
            utf32.push( ((unit-0xD800)<<10) + (low-0xDC00) + 0x10000 );
            i += 2;
        } else { return nullptr; }
